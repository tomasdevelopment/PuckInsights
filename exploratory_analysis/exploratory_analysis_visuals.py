...
def plot_points_hist(df, col='points', bins=15, title='Histogram of Player Points'):
    ...
    """
    ax = df[col].plot.hist(bins=bins, figsize=(5,4), alpha=0.7, edgecolor='black')
...
def binned_points_distribution(df, col='points', bins=10):
    """
    Returns the value counts for points binned into intervals.
    """
    binned = pd.cut(df[col], bins=bins)
    return binned.value_counts().sort_index()

def plot_points_hist_kde(df, col='points', bins=30, title='Points Distribution (Histogram + KDE)'):
    """
    ...
    plt.show()
    return ax
#olot top teams by save percentage
def plot_top_save_pct_teams_year(df, col_team='team', col_save='save_percentage', col_year='year', top_n=10):
    """
    ...
    group_avg = subset.groupby([col_team, col_year])[col_save].mean().sort_values(ascending=False)
    top_group = group_avg.head(top_n)
    # Make readable labels like "TeamName (Year)"
